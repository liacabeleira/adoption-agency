			...
			<div id="wrapper">
		        ...
		        <%
					    ...
					    try (Connection cn = ligacaomysql.criarligacao()) {
					        String query = "SELECT name_animal FROM animals WHERE id_animal=?";
					        try (PreparedStatement ps = cn.prepareStatement(query)) {
					            ps.setString(1, idAnimal);
					            
					            try (ResultSet rs = ps.executeQuery()) {
					                if (rs.next()) {
					                    nameAnimal = rs.getString("name_animal");
					                } else {
					                    out.println("<p style='color: red;'>Animal not found!</p>");
					                    return;
					                }
					            }
					        }
					    } catch (Exception e) {
					        out.println("<p style='color: red;'>Error fetching animal details: " + e.getMessage() + "</p>");
					        return;
					    }
					%>


					<p>Are you sure you want to delete the animal: <strong><%= nameAnimal %></strong>?</p>
					
					<!-- Botões para confirmação ou cancelamento -->
					<div style="display: flex; gap: 10px; align-items: center;">
					
					    <!-- Botão para confirmação -->
					    <form action="confirm_delete_animal.jsp" method="POST" style="margin: 0;">
