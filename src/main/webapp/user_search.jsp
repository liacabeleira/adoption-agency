			        ...
			        <header id="header">
				        ...
				        <%
				            ...
				            if (emailUser != null && !emailUser.isEmpty()) {
				             if (Validator.isValidEmail(emailUser)) {
				                try {
				                    // Conexão com o banco de dados
				                    Connection cn = ligacaomysql.criarligacao();
				                    String sql = "SELECT id_user, firstname, surname, email, address, zip_code, city, country, phone_number, role, status_user FROM user WHERE email = ?";
				                    ...
				                    if (rs.next()) {
				                        ...
				                        out.println("<div class='message' style='color: black;'>User not found</div>");
				                    }
				                    cn.close();
				                } catch (Exception e) {
				                    e.printStackTrace();
				                    out.println("<div class='message' style='color: black;'>Error: " + e.getMessage() + "</div>");
				                }
				            } else {
				                // Mensagem de email inválido
				                out.println("<div class='message' style='color: black;'>Invalid email format</div>");
				            }
				        } else {
				            out.println("<div class='message' style='color: black;'>Please enter a valid email</div>");
				        }
				        %>
