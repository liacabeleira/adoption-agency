			...
			<div id="wrapper">
		        ...
		        <div style="margin: 20px auto; max-width: 600px; text-align: center;">
					 <%
					    ...
					    String nameAnimal = request.getParameter("new_name_animal");
					    String gender = request.getParameter("new_gender");
					    String idSpecies = request.getParameter("new_id_species");
					    String statusAnimal = request.getParameter("new_status_animal");
					    boolean updateSuccess = false;
					    int userId = (int) session.getAttribute("id_user");
					    
					 	// Sanitiza dados do input
				      	nameAnimal = Validator.sanitizeInput(nameAnimal);
					
					    if (Validator.isValidName(nameAnimal)) {
					        try (Connection cn = ligacaomysql.criarligacao()) {
					            // Atualizar a tabela species
					            String query = "UPDATE animals SET name_animal=?, gender=?, id_species=?, status_animal=? WHERE id_animal = ?";
					            try (PreparedStatement ps = cn.prepareStatement(query)) {
					                ps.setString(1, nameAnimal);
					                ps.setString(2, gender);
					                ps.setString(3, idSpecies);
					                ps.setString(4, statusAnimal);
					                ps.setString(5, idAnimal);
					            ...
					            if (updateSuccess) {
					                ...
					                try (PreparedStatement logPs = cn.prepareStatement(logQuery)) {
					                    ...
					                    logPs.setString(2, "Updated Animal ID " + idAnimal + " details");
					                    logPs.executeUpdate();
					                }
					            }
					        } catch (Exception e) {
					            out.println("<p style='color: red;'>Error updating animal: " + e.getMessage() + "</p>");
					        }
					    } else {
					        out.println("<p style='color: red;'>Invalid animal name. Please ensure it is at least 2 characters long and contains only letters and spaces.</p>");
					    }
