			...
			<div id="wrapper">
		        ...
		        <header id="header">
		        	...
		        	<%
						    ...
						    // Excluir o animal e registar no log
						    try (Connection cn = ligacaomysql.criarligacao()) {
						        // Excluir o animal
						        String deleteQuery = "DELETE FROM animals WHERE id_animal = ?";
						        ...
						        if (deleteSuccess) {
						            ...
						            try (PreparedStatement logPs = cn.prepareStatement(logQuery)) {
						                ...
						                logPs.setString(2, "Deleted Animal ID " + idAnimal);
						                logPs.executeUpdate();
						            }
						        }
						    } catch (Exception e) {
						        out.println("<p style='color: red;'>Error deleting animal: " + e.getMessage() + "</p>");
						    }
						%>
						...
						<%
						    if (deleteSuccess) {
						%>
						<p style="color: green;">Animal deleted successfully!</p>
						...
						%>
						<p style="color: red;">Failed to delete animal. It may be associated with other records.</p>
						<button onclick="window.location.href='animals.jsp'" style="padding: 10px 20px; background-color: #f44336; color: white; border: none; cursor: pointer; border-radius: 4px;">
						    Back
						</button>
						<%
						    }
