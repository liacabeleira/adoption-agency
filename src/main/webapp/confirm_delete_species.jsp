		        ...
		        <header id="header">
		            <h1>Delete Species</h1>  
		        		...
		        		<%
						    ...
						    // Excluir a espécie e registar no log
						    try (Connection cn = ligacaomysql.criarligacao()) {
						        // Excluir a espécie
						        String deleteQuery = "DELETE FROM species WHERE id_species = ?";
						        ...
						        if (deleteSuccess) {
						            ...
						            try (PreparedStatement logPs = cn.prepareStatement(logQuery)) {
						                ...
						                logPs.setString(2, "Deleted species ID " + idSpecies);
						                logPs.executeUpdate();
						            }
						        }
						    } catch (Exception e) {
						        out.println("<p style='color: red;'>Error deleting species: " + e.getMessage() + "</p>");
						    }
						%>
						...
						<%
						    if (deleteSuccess) {
						%>
						<p style="color: green;">Species deleted successfully!</p>
						...
						%>
						<p style="color: red;">Failed to delete species. It may be associated with other records.</p>
						<button onclick="window.location.href='species.jsp'" style="padding: 10px 20px; background-color: #f44336; color: white; border: none; cursor: pointer; border-radius: 4px;">
						    Back
						</button>
						<%
						    }
